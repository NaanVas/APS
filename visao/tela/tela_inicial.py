...
class TelaInicial(TelaPadrao):
    ...
    def menu_usuario(self, tipo_usuario):
        ...
        self.botao_emprestimo = ttk.Button(self.frame_botoes, text="Empréstimo", style="Estilo.TButton", command=lambda: self.abir_tela_listar_emprestimos(tipo_usuario))
        ...
        botao_sair.grid(row=3, column=0, pady=10, padx=10, sticky="ew")
    ...
    def menu_funcionario(self, tipo_usuario):
        ...
        self.botao_emprestimo = ttk.Button(self.frame_botoes, text="Empréstimo", style="Estilo.TButton", command=self.abrir_tela_emprestimo(tipo_usuario))
        self.botao_emprestimo.grid(row=2, column=0, pady=10, padx=10, sticky="ew")
    ...
    def abrir_tela_emprestimo(self, tipo_usuario):
        self.janela.withdraw()
        self.janela_emprestimo = TelaEmprestimoMenu(self.root, self.voltar_tela_inicial, tipo_usuario, self.cpf)

    def abir_tela_listar_emprestimos(self, tipo_usuario):
        if self.janela_listar_aberta:
            self.janela_listar_aberta.fechar_tela()
        self.janela_listar_aberta = TelaListarEmprestimos(self.root, tipo_usuario, self.cpf)
