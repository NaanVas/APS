...
from visao.tela.usuario.cadastro import TelaCadastroUsuario
from visao.tela.usuario.excluir import TelaExcluirUsuario
from visao.tela.usuario.listar import TelaListarUsuarios
from visao.tela.usuario.buscar import TelaBuscarUsuario
...
class TelaInicialUsuario(TelaPadrao):
    def __init__(self, root, voltar_callback):
        ...
        label_titulo = tk.Label(self.frame_central, text="Tela de Gerenciamento de Usuários", font=("Montserrat", 16, "bold"), fg="#893F04", bg="#E5E0D8")
        label_titulo.grid(row=0, column=0, columnspan=2, pady=20)

        self.frame_botoes.grid(row=1, column=0, columnspan=2, pady=20)

        botao_buscar = ttk.Button(self.frame_botoes, text="Buscar Usuário", style="Estilo.TButton", command=self.abrir_tela_buscar)
        botao_buscar.grid(row=0, column=0, pady=10, columnspan=2,padx=10, sticky="ew")

        botao_listar = ttk.Button(self.frame_botoes, text="Listar Usuários",style="Estilo.TButton", command=self.abrir_tela_listar)
        botao_listar.grid(row=1, column=0, pady=10, columnspan=2,padx=10, sticky="ew")

        botao_cadastrar = ttk.Button(self.frame_botoes, text="Cadastrar Usuário",style="Estilo.TButton", command=self.abrir_tela_cadastrar)
        botao_cadastrar.grid(row=2, column=0, pady=10, columnspan=2,padx=10, sticky="ew")

        botao_excluir = ttk.Button(self.frame_botoes, text="Excluir Usuário",style="Estilo.TButton", command=self.abrir_tela_excluir)
